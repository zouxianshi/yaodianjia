<template>
  <div class="phone-view-modal">
    <div class="header">
      <span> &emsp;‹返回 </span>
      <span class="title">优惠券</span>
      <span class="title">...&emsp;</span>
    </div>
    <div class="body">
      <div class="content">
        <div class="algin-center">
          <img src="https://shadow.elemecdn.com/app/element/hamburger.9cf7b091-55e9-11e9-a976-7f4d0b07eef6.png">
          <div>中智大药房</div>
          <div class="quanName">{{ data.name || '未命名' }}</div>
          <div><el-button type="success" size="mini" @click="check">立即使用</el-button></div>
        </div>
        <div class="algin-left">
          <div>使用条件：{{ useRules }}</div>
          <div>可用时间：{{ userTime }}</div>
          <div>可用门店：{{ data.store === '0' ? '全部门店':'部分门店' }}</div>
          <div>使用须知：{{ data.notice }}</div>
        </div>
        <div class="p-bottom">
          <div class="codeImg">二维码</div>
          <div class="code">券码：12345678</div>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  name: 'PhoneView',
  props: {
    data: {
      type: Object,
      default() {
        return {}
      }
    },
    otherData: {
      type: Object,
      default() {
        return {}
      }
    }
  },
  computed: {
    useRules() {
      var str = ''
      str += this.data.scenes === '0' ? '线上线下通用,' : this.data.scenes === '1' ? '微商城使用,' : '线下门店使用,'
      str += this.data.threshold === '0' ? '订单金额无门槛' : '订单金额满' + this.data.thresholdPrice + '元可使用'
      return str
    },
    userTime() {
      var str = ''
      if (this.data.expiration === '0') { // 选择马上开始有效期
        str += '自领取起' + this.otherData.expirationDay + '天内有效'
      } else if (this.data.expiration === '1') {
        str += '自领取' + this.otherData.notActive + '天后' + this.otherData.effective + '天有效'
      } else { // 选择日期
        str += this.formatDate(this.otherData.expirationDate[0]) + '到' + this.formatDate(this.otherData.expirationDate[1])
      }
      return str
    }
  },
  methods: {
    check() {
      console.log(this.data, this.otherData)
    },
    formatDate(date) {
      var y = date.getFullYear()
      var m = date.getMonth() + 1
      m = m < 10 ? '0' + m : m
      var d = date.getDate()
      d = d < 10 ? ('0' + d) : d
      return y + '年' + m + '月' + d
    }
  }
}
</script>
<style lang="scss" scoped>
.phone-view-modal {
  border: 1px solid #aaa;height:80%;box-sizing:border-box;
  .header {
    background-color: #333;
    color: #fff;
    display: flex;
    justify-content: space-between;
    height: 40px;line-height:40px;
    .title{
      font-weight: bold;font-size: 16px;
    }
  }
  .body{
    height: calc(100% - 40px);padding: 20px;box-sizing:border-box;
    .content{
      border: 1px solid #aaa;height: 100%;position:relative;
      .algin-center{
        text-align: center;width: 100%;padding-top:5%;margin-bottom: 20px;
        img{
          height: 80px;width: 80px;border-radius: 50%;border:1px solid #eee;
        }
        div{
          margin-top: 10px;
        }
        .quanName{
          font-size: 16px;font-weight: bold;
        }
      }
      .algin-left{
        padding: 0 10%;
        line-height: 24px;
      }
      .p-bottom{
        position: absolute;bottom: 0;text-align: center;height: 130px;width: 100%;
        padding-top: 10px;border-top: 2px dotted #999;
        .codeImg{
          height: 80px;border: 1px solid #eee;width: 80%;margin: 0 auto 15px;
        }
        .code{
          font-size: 12px;
        }
      }
    }
  }
}
</style>
