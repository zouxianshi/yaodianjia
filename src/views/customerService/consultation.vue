<template>
  <chat-room v-loading="consultingLoading" :cur-user-name="curUserName">
    <template slot="userList">
      <div class="chat-list">
        <div class="search-box">
          <el-input id v-model="searchText" size="mini" class="search-input" type="search" maxlength="30" name placeholder="请输入客户名称" @input="handleSearchInput">
    ...
    <template slot="chat-window-main">
      <div id="chat-detail-list" v-loading="messageLoading" class="chat-detail-list">
        ...
        <template v-if="curOnlineUserData.list.length>0">
          ...
          <div v-for="(dItem,index) in curOnlineUserData.list" :key="index" :class="`chat-item-box`">
            ...
            <div :class="`chat-item-inner ${dItem.fromUserId == targetId? '': 'right-align'}`">
              ...
              <div :class="`chat-item-content ${computeChatItemType(dItem.messageType)}`">
                <div v-if="dItem.messageType===MessageType.TextMessage" class="text-message">{{ symbolToEmoji(dItem.content) }}</div>
                <div v-else-if="dItem.messageType===MessageType.ImageMessage" class="image-message">
                  <el-image fit="scale-down" :src="dItem.content" :preview-src-list="[dItem.content]" />
                </div>
                <div v-else-if="dItem.messageType===MessageType.GoodsMessage" class="goods-message" @click="handleGoodsClick(dItem)">
              ...
              <!-- 客服发的头像靠右 -->
              <div v-if="dItem.fromUserId !== targetId" class="chat-item-avatar">
                <el-image fit="scale-down" :src="merLogo" />
              </div>
            </div>
          </div>
        </template>
        ...
        <div class="action-list">
          <el-popover v-model="emojiPopVisible" placement="top-start" width="350" trigger="click">
            <div class="emoji-list">
              <span v-for="(item,index) in emojiList" :key="index" class="emoji-item" @click="emojiItemClick(item)">{{ item.emoji }}</span>
            </div>
            <i slot="reference" class="el-icon-picture-outline-round action-item" />
          </el-popover>
          <input id="pic" type="file" name="pic" accept="image/gif, image/jpeg, image/png, image/webp" @change="handleChange">
          ...
          <el-popover v-model="cannedRepliesVisible" placement="top" width="200" trigger="click">
            <div class="canned-msg-list">
              <div v-for="(item,index) in cannedResList" :key="index" class="canned-msg-item" @click="msgItemClick(item)">{{ item.msg }}{{ item.picture ? '【图片】' : '' }}</div>
            </div>
            <!-- <div class="add-reply-btn" type="text" @click="addCannedReply">新增快捷回复</div> -->
            <i slot="reference" class="el-icon-chat-square action-item" />
          </el-popover>
          <el-dialog :close-on-click-modal="false" append-to-body title="确认发送" :visible.sync="sendCannedMsgDialogVisible" width="30%">
            <template v-if="selectedCannedMsg">
              <div class="canned-msg-text">{{ selectedCannedMsg.msg }}</div>
              <div v-if="selectedCannedMsg.picture" class="canned-msg-picture">
                <el-image :src="selectedCannedMsg.picture" />
              </div>
            </template>
            <span slot="footer" class="dialog-footer">
              <el-button @click="sendCannedMsgDialogVisible = false">取 消</el-button>
              <el-button type="primary" @click="handleCannedMsgSendClick()">确 定</el-button>
            </span>
          </el-dialog>
          <i class="el-icon-shopping-bag-2 action-item" @click="handleActionClick('goods')" />
          <el-dialog title="选择商品" :visible.sync="selectGoodsDialogVisible" append-to-body :close-on-click-modal="false" :before-close="handleSelectGoodsDialogClose">
            <div class="search-filter-item">
              <el-input v-model="goodsQuery.name" class="search-filter-item-input" type="text" placeholder="请输入关键字搜索" maxlength="30" @input="handleGoodsNameInput" />
              <el-button class="search-filter-btn" type="primary" size="small" @click="goodsSearchBtnClick">搜索</el-button>
            </div>
            <el-table ref="multipleTable" fit :data="goodsList" tooltip-effect="dark" height="300">
              <el-table-column fixed="left" prop="image" label="图片" width="150">
                <template slot-scope="scope">
                  <el-image fit="scale-down" :src="showImg(scope.row.mainPic)" :preview-src-list="[showImg(scope.row.mainPic)]" />
                </template>
              </el-table-column>
              ...
              <el-table-column prop="packStandard" label="规格" width="150">
                <template slot-scope="scope">
                  <div v-if="scope.row.specSkuList && scope.row.specSkuList.length > 0">
                    <div v-for="(item,index) in scope.row.specSkuList" :key="index" class="inner">{{ item.skuKeyName }}:{{ item.skuValue }}</div>
                  </div>
                </template>
              </el-table-column>
            ...
            <el-pagination class="pagination" :current-page="goodsQuery.currentPage" :page-sizes="goodsPagination.pageSizes" :page-size="goodsQuery.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="goodsPagination.total" @size-change="handleGoodsDialogSizeChange" @current-change="handleGoodsDialogCurPageChange" />
          </el-dialog>
        </div>
        <div class="text-area">
          <el-input v-model="textMsgValue" class type="textarea" rows="5" placeholder="请输入内容" maxlength="512" @input="handleTextAreaChange" @keyup.enter.native="handleTextAreaKeyup" />
        </div>
    ...
    <template slot="chat-user-info">
      <user-info :member-info="memberInfo" :bought-record="boughtRecord" :order-list="orderList" :order-list-has-more="orderListHasMore" :order-list-loading="orderListLoading" @loadOrderData="queryUserOrderList" />
    </template>
  </chat-room>
</template>
