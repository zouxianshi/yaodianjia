    ...
    <div class="scrollbar sscm-select-box">
      <el-form label-width="80px">
        <el-form-item label="已选">
          <el-tag v-for="(items,$index) in selectList" :key="$index">{{ items.cname }}</el-tag>
        </el-form-item>
      </el-form>
    </div>
    <div class="sscm-condition-box">
      <span class="sscm-coupon-type">
        ...
        <el-select v-model="searchParams.ctype" size="mini" placeholder="请选择" style="width: 200px;">
          <el-option v-for="items in [{value:0,label:'全部'},{value:1,label:'折扣券'},{value:2,label:'满减劵'},{value:3,label:'礼品券'},]" :key="items.value" :label="items.label" :value="items.value" />
        </el-select>
      </span>
