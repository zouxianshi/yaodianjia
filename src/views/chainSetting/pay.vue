    ...
    <div class="border" style="display:inline-block;width: 100%;margin-top: 10px">
      <div>
        ...
        <el-checkbox
          ...
          :true-label="1"
          :false-label="0"
          @change="changeOrderPayset"
        >货到付款</el-checkbox>
      </div>
      <div style="margin-top: 5px">
        <span class="label-name">配送订单：</span>
        <el-checkbox
          v-model="distributionOrder"
          :true-label="1"
          :false-label="0"
          @change="changeOrderPayset"
        >货到付款</el-checkbox>
      ...
      <el-alert type="warning" :closable="false" style="line-height:20px;margin-top:10px">
        后台所有订单都支持在线支付，自提订单支持在线支付及到店支付。
        <br>此处选择商户自行配置，提示：为了避免顾客与商家之间关于付款或退款的纠纷，建议商家送货上门的订单不要选择货到付款
      </el-alert>
        ...
        <el-form-item label="支付授权配置：">
          <span>
            ...
            <el-button
              ...
              class="copy-key"
              :data-clipboard-text="form.payAutoConfig"
              @click="onCopyLink"
            >复制</el-button>
            <el-popover placement="right" title="操作提示：" width="500" trigger="click">
              <span style="line-height: 30px">
                1：进入微信商户平台
                <br>2：点击“产品中心”，点击“开发配置”
                <br>3：找到支付配置，公众号支付配置，点击“添加”
                <br>4：选择左侧相应的支付授权链接复制
                <br>5：粘贴到输入框中,
                <el-button type="text" @click="showGuide(4)">点击查看图片</el-button>
                <br>6：支付授权目录开始下拉框切记选择https://
              </span>
              <el-image
                id="imgWechatGuide4"
          ...
          <el-popover placement="right" title="操作提示：" width="500" trigger="click">
            <span style="line-height: 30px">
              ...
              <el-link
                ...
                :underline="false"
                target="_blank"
              >https://pay.weixin.qq.com/</el-link>
              <br>2：点击“微信支付”，复制微信支付商户号
              <br>3：粘贴到输入框中,
          ...
          <el-popover placement="right" title="操作提示：" width="500" trigger="click">
            <span style="line-height: 30px">
              ...
              <el-link
                ...
                target="_blank"
              >https://pay.weixin.qq.com/</el-link>
              <br>2：点击“API安全”，根据要求安装相关证书，已安装略过
              <br>3：点击“设置秘钥”，
              ...
              >
                <span style="line-height: 30px">
                  ...
                  <el-link
                    ...
                    target="_blank"
                  >https://pay.weixin.qq.com/</el-link>
                  <br>2：进入账户中心>API安全>下载证书
                  <br>3：
                  <el-button type="text" @click="showGuide(3)">点击查看图片</el-button>
                </span>
                <el-image
                  id="imgWechatGuide3"
                  style="width: 0px; height: 0px"
...
export default {
  ...
  data() {
    return {
      ...
      wechatGuideList1: [
        require('../../assets/image/chainSetting/img_wechat_guide1.png')
      ],
      wechatGuideList2: [
        require('../../assets/image/chainSetting/img_wechat_guide2.jpg')
      ],
      wechatGuideList3: [
        require('../../assets/image/chainSetting/img_wechat_guide3.png')
      ],
      wechatGuideList4: [
        require('../../assets/image/chainSetting/img_wechat_guide4.jpg')
      ],
      rules: {
        merchantCode: [
          { required: true, message: '请输入支付商户号', trgger: 'blur' }
        ],
        payKey: [{ required: true, message: '请输入支付key', trgger: 'blur' }],
        merchantCertificate: [
  ...
  methods: {
    ...
    getPayset() {
      ...
      getPayset({
        ...
        if (res.code === '10000') {
          ...
          const tempData = _.filter(_.cloneDeep(res.data), o => {
            return o.payType === 0
          })
    ...
    setStatus(value) {
      ...
      // 如果要关闭微信支付，那么必须勾选所有订单支付方式
      if (this.form.status === 0) {
        if (!this.deliveryOrder || !this.distributionOrder || !this.selfOrder) {
          this.$message({
            message: '关闭微信支付需先勾选所有线下支付方式！',
            type: 'error',
            duration: 5 * 1000
          })
          this.form.status = 1
