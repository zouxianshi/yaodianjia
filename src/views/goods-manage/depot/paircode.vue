<template>
  <div class="app-container">
    <div class>
      ...
      <el-radio-group v-model="listQuery.status" size="small" @change="getList">
      ...
      <div class="table-box">
        ...
        >
          ...
          <el-table-column align="left" min-width="120" label="商品名称" show-overflow-tooltip>
            <template slot-scope="scope">
              <div>
                <p class="ellipsis" v-text="scope.row.name" />
                <p
                  v-if="listQuery.status===1&&scope.row.product"
          ...
          <el-table-column align="left" min-width="180" label="生产企业" show-overflow-tooltip>
            <template slot-scope="scope">
              <div>
                ...
                <p
                  v-if="listQuery.status===1&&scope.row.product"
                  class="product ellipsis"
                  v-text="scope.row.product.manufacture"
                />
              </div>
            </template>
          </el-table-column>
          <el-table-column align="left" label="条形码" :show-overflow-tooltip="true" min-width="120">
            <template slot-scope="scope">
              <div>
                ...
                <p
                  v-if="listQuery.status===1&&scope.row.product"
                  class="product"
                  v-text="scope.row.product.barCode"
                />
              </div>
            </template>
          </el-table-column>
          <el-table-column align="left" label="批准文号" :show-overflow-tooltip="true" min-width="130">
            <template slot-scope="scope">
              <div class="ellipsis">
                ...
                <p
                  v-if="listQuery.status===1&&scope.row.product"
                  class="product ellipsis"
                  v-text="scope.row.product.approvalNumber"
                />
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column
            ...
            label="导入时间"
          />
          <el-table-column prop="modifyName" align="left" min-width="120" label="操作人" />
          <el-table-column
...
import { getImportList, deletePair, exportNotMatch } from '@/api/depot'
...
export default {
  ...
  methods: {
    ...
    handleMate(row) {
      ...
      this.$router.push(
        `/goods-manage/mate-details?id=${row.id}&from=${
          this.listQuery.status === 1 ? 'is_pair' : 'pair'
        }`
      )
    },
